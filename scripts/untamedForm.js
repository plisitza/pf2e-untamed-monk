...
let formGroups = [ // contains Attack info for the forms
    {
        group: "Untamed Form: Pest Form", // Doubles as the Feat Name and the Heading under which the form options are displayed in the selection menu
    ...
    {
        group: "Untamed Form: Animal Form", // Doubles as the Feat Name and the Heading under which the form options are displayed in the selection menu
...
async function removeCustomMods() {
    ...
    for (let element of customMods) {
        for (let mod of actor.system.customModifiers[element]) {
            if ((mod.label ?? mod.name ?? "").includes("WSForm")) {
...
async function applyResistances(formData) {
    ...
    const oldId = actor.getFlag("world", "ws_effectId");
    if (oldId && actor.items.get(oldId)) {
        await actor.deleteEmbeddedDocuments("Item", [oldId]);
    }
    if (rules.length) {
        const effectSource = {
            name: `Effect: ${formData.name} Form (Untamed)`,
            type: "effect",
        ...
        await actor.setFlag("world", "ws_effectId", created.id);
    }
}
...
async function resetSpeeds() {
    ...
    // Clear any ws_origSpeeds flag left behind by an older version of this module.
    if (actor.getFlag("world", "ws_origSpeeds") !== undefined) {
...
async function resetSenses() {
    const effectId = actor.getFlag("world", "ws_effectId");
    if (effectId && actor.items.get(effectId)) {
        await actor.deleteEmbeddedDocuments("Item", [effectId]);
    }
    await actor.unsetFlag("world", "ws_effectId");
...
async function transform(html) {
        ...
        if (!formData) {
            ...
            // but visible as a grow/shrink/grow flicker once CreatureSize and BaseSpeed moved onto
            // it. The dragon-resistance picker still rebuilds it deliberately, which is correct:
            // its rules genuinely change.
            await applyResistances(formData)
...
let wsFeats = [];
const allFeats = (actor.items.contents).filter(item => item.type === "feat") 
for (let i=0; i<allFeats.length; i++) {
    ...
    // Accept both remaster and legacy feat names (Wild Shape = Untamed Form)
    let featName = allFeats[i].name === "Wild Shape" ? "Untamed Form" : allFeats[i].name;
    let foundFeat = featNames.filter(name => name.includes(featName))
...
let heightenedLevel = Math.ceil((actor.system.details.level.value) / 2); 
